...
export default function SalesOrderRow({ order, ptAccess }) {
  ...
  return (
    <tr className="transition-colors hover:bg-[#fafafd]">
      ...
      <td className="px-4 py-3 align-middle whitespace-nowrap text-ink text-xs"><a className="px-3 py-2 bg-canvas rounded-md" href={`/sales-orders/${order.so_id}?so_number=${order.so_number}&pelanggan=${order.customer_name}&status=${order.overall_status}&pt=${ptAccess}`}>Detail</a></td>
    </tr>
  )
}
